...
from SWRT_Zeng_ex14_LockedModes import EigenRead, single_freq, Csingle_freq, Csingle_freq2, Csingle_freqA, Csingle_freqPQ
...
# for Mode in ['10']:
    ...
    # dir_SPECFEM='/uufs/chpc.utah.edu/common/home/flin-group5/qicheng/specfem2d/EXAMPLES/RayleighWaveGJI/OUTPUT_SRP_SOURCEL1000km/SAC/'
    ...
    suffix='wavefieldGJI'   #dirM.split('/')[-1]
    ...
    if SOURCE=='L':
        ...
        X_S=X_SR
    dir_rec=dir_SK+'OUTPUT_SRP_SOURCE'+SOURCE+'%dkm'%X_S+'_Lock_'+suffix+'_Mode'+Mode+'_WithRef0_10'
    ...
    def ToSACModes(dir_rec,sta,dist1,SOURCE,Mode):
        ...
        for m in range(NModes):
            ...
            if SOURCE=='L':
                X_S=X_SL
                st+=read(dir_NormalModes+'S1501.EHZ.sac_XS%dkm'%X_S)
            else:
                X_S=X_SR
                st+=read(dir_NormalModes+'S2501.EHZ.sac_XS%dkm'%X_S)
        ...
        for m in range(NModes):
            ...
            data_inc=TaperCos(data_inc,tt,tb_inc,te_inc)
            if dist1<=distC: #behaviour exactly at boundary? 
                st1_inc[m].data=TimeShiftDisper(data_inc, tt, 
            ...
            uz0.data=np.zeros(uz.data.shape)
            uz0.stats.sac.dist=dist1
            if Mode=='0':
                uz0.data=st1[0].data
            if Mode=='9':
        ...
        uz0.write(dir_rec+'/SAC/'+sta+'.'+COMP+'.sac',format='SAC') #!!! Writing to disk
        # uz0.write(dir_rec+'/SAC/'+sta+'.'+COMP+'.sac',format='SAC') #!!! Writing to disk
        # Uz.write(dir_rec+'/SAC/'+sta+'.'+COMP+'.sac',format='SAC') #!!! Writing to disk
        # return tt, uz,data_inc,dist1,dist,st
        return tt, uz0, st1_inc, st1_ref, st1, data_inc,st
...
#%%
# tt, uz0, st1_inc, st1_ref, st1, data_inc,st=ToSACModes(dir_rec,'S1501',1500.0,'L','0')
# #%%
# plt.figure()
# # plt.plot(tt,uz.data)
# plt.plot(tt,uz0.data)
